...
export const normalizeCard = (card, index = 0, transactions = []) => {
  ...
  return {
    ...
    benefitItems: preferredCard.benefitItems || preferredCard.benefit_items || [],
    benefits: preferredCard.benefits || [],
    officialUrl: preferredCard.officialUrl || preferredCard.official_url || preferredCard.naverUrl || preferredCard.naver_url || '',
    naverUrl: preferredCard.naverUrl || preferredCard.naver_url || '',
    ...
    ownedCategoryGuides: [
      {
        ...
        potentialBenefit: 3000,
        monthlyGain: 0,
        benefitLabel: '카페·음식점 할인',
        eligibleForBenefit: true,
        nextMonthEligible: false,
        remainingCurrentSpend: 183200,
        scope: 'owned',
        scopeLabel: '보유 카드 추천',
        title: '외식은 LOCA LIKIT Eat',
        body: '외식 지출은 보유 카드 중 LOCA LIKIT Eat가 가장 잘 맞습니다.',
      },
      {
        id: 'owned-category-shopping-10612',
        category: '쇼핑',
        amount: 79650,
        cardId: '10612',
        cardName: '카드의정석2 SHOPPER',
        issuer: '우리카드',
        imageUrl: '/card-images/10612.png',
        estimatedBenefit: 7965,
        potentialBenefit: 7965,
        monthlyGain: 0,
        benefitLabel: '온라인 쇼핑 할인',
        eligibleForBenefit: true,
        nextMonthEligible: true,
        remainingCurrentSpend: 0,
        scope: 'owned',
        scopeLabel: '보유 카드 추천',
        title: '쇼핑은 카드의정석2 SHOPPER',
        body: '쇼핑 지출은 보유 카드 중 카드의정석2 SHOPPER가 가장 잘 맞습니다.',
      },
    ],
    results,
...
export async function fetchCardRecommendationBundle({ recurringCategories = [] } = {}) {
  ...
  const normalizeRecommendationBundle = (data) => ({
    ...
    ownedCategoryGuides: (data.ownedCategoryGuides || []).map((item) => ({
      ...item,
      imageUrl: preferredCardImageUrl({ id: item.cardId, cardAdId: item.cardId }, item.imageUrl),
    })),
